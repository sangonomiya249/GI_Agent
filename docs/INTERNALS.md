...
* **只跑一部分不算**：这种材料的路线至少要跑掉 80%（`GATHER_COOLDOWN_MIN_ROUTE_PERCENT`）才算"采完"。
...
* **跨自然日**：日志按天切文件，48 小时横跨 3 天，所以往前扫 3 个文件。
* **三层拦截**：① 每轮上下文注入【采集物冷却】表 → 模型据此改推别的材料；
  ② 审批屏多一行 `⏳ …还要等约 N 小时`；③ 执行前 `_filter_gather_cooldown()` 把冷却中的目标摘掉，
  **摘光了就直接跳过启动 BetterGI**（空计划保护）。
...
> 这也是把它做成开关的原因。
>
> **冷却判定会跟着开关走**：开隔离带时按"跑了几条 / 一共几条"判（只跑 1 条隔离带路线不算采完）；
