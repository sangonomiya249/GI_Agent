...
* **面板只列"你的脚本组里有的"**：冷却表的数据源是脚本组（见上），所以
  「食材与炼金」可能只列出一个久雨莲 —— 玩家实测问过这个（"实际还有好多采集，是读不到吗"），
  答案是**读得到，但那个组里只有久雨莲的路线**（他的 `AutoPathing/食材与炼金` 下有 61 个材料目录）。
  这类材料由 `pathing_materials_all()` 从路线仓库目录读出来（地方特产要再深一层：
  `地方特产/<地区>/<材料>`，其余是 `<类目>/<材料>`；带括号备注的名字会剥掉，
  「提瓦特食材一条龙」这种整包脚本会被过滤），`unsubscribed_materials()` 取它减去组内词表的差集 ——
  面板页脚列出来提醒，**不进冷却表**：没有脚本组就没有路线可开关，Agent 也跑不了它们。
  实测本机：矿物 3/8（差 5）、食材与炼金 1/59（差 58）、敌人与魔物 11/55（差 44）、地区特产 59/59（差 0）。
