...
* **采集冷却**（`🌿`）：地图特产的 48 小时冷却一览 —— 四种汇总卡（冷却中 / 可以采 / 手动登记 /
  判定依据）+ 一张表：材料 / 状态（`冷却中 · 还要 20 小时`、`可以采`、`部分采集`）/ 上次采集时间 /
  路线进度（`11/11 条`）；
  ...
  * ⚠️ 页面只列**采集类目**（地图素材 / 矿物 / 食材与炼金）的材料：敌人与魔物那些组的"材料"
    其实是魔物名（巡陆艇、蕈兽…），不是 48 小时刷新的特产，混进来这页就没法看了；
