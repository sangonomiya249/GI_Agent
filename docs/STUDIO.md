...
* **概览页 — 版本与更新卡**：本地版本（仓库根的 `VERSION`，没有就退回 `git describe`）、
  最新 release 的 tag / 发布时间 / 发布页链接 / 更新说明、以及一句结论
  （`有新版本` / `已是最新` / `本地比 release 新` / `检查失败`）。
  点右上角**「检查更新」**忽略 6 小时缓存立刻重查；平时打开概览页只读缓存，
  免得每次开页面都打 GitHub 的匿名接口（每小时 60 次）。
  数据来自 `GET /api/update`（`?force=1` 强制重查）→ `skills/update_check.check()`。
  **只读**：不下载、不改文件，更新方式只给命令（git 用户 `git pull`，zip 用户去发布页下载）。
  离线 / 被墙 / 限流时这张卡只显示一条失败原因，其余功能照常。
