...
│ Studio   ├──────────────────────────────────────────────────────────────────┤
│ ▶️ 运行   │  ┌ Agent ─┐ ┌ LLM ─┐ ┌ BetterGI ─┐ ┌ 一条龙 ─┐ ┌ 展柜 ─┐ ┌ 路线 ┐ │
│ 🏠 概览   │  └────────┘ └──────┘ └───────────┘ └─────────┘ └───────┘ └──────┘ │
│ 🌿 冷却   │  ┌ 本轮将执行 ─────────────┐ ┌ BetterGI 最近日志 ──────────────┐ │
│ 🌱 养成   │  │ ⚔️ 体力目标：无          │ │ ⚠️ 第 1234 行：战斗策略缺失      │ │
│ 🗺️ 任务   │  │ 🌿 采集：清心  👹 蕈兽   │ │ …（等宽字体、按级别上色）        │ │
│ 🔌 通道   │  └──────────────────────────┘ └──────────────────────────────────┘ │
│ ── 系统 ──│  ┌ 快捷操作：刷新展柜 / 体检 / 打开目录 / 回滚配置 ─────────────────┐ │
│ ⚙️ 配置   │  ⏻ 退出 Studio                                                     │
│ 📖 说明   │                                                                    │
│ 🩺 体检   │                                                                    │
│ 📜 日志   │                                                                    │
...
* **角色养成页**（`🌱`）：米游社算需求 / 读真实库存，缺口转成 BetterGI 任务。
  四张状态卡（养成计划 / 米游社库存 / 当前角色 / 今日预计体力）+ 库存同步卡（**库存过期时明确写"这不是实时数据"**）
  + 任务表（状态 / 角色阶段 / 材料 / 缺口 / 来源 / 说明）+ 角色目标表（拖拽排序、启用禁用、编辑目标）
  + 同步与执行历史。右上角四个按钮：**同步米游社库存**、**重新计算缺口**、**刷新页面数据**、**开始执行**
  （可勾「只写配置（不启动）」）。数据来自 `GET /api/growth`，详细说明见 [角色养成](GROWTH.md)。

* **版本更新页**（`🔄` 在左侧「系统」栏里）：本地版本、最新 release、状态、这次是从哪条路查到的
  （代理 / 证书），下面一张「更新说明」卡贴 release 正文。
  右上角两个按钮：**「检查更新」**忽略缓存立刻重查、**「打开发布页」**用系统浏览器打开 release 页面
  （走 `/api/open`，只放行 GitHub 链接）。
  有新版本时左侧这一项会亮小红点。
  数据来自 `GET /api/update`（`?force=1` 强制重查）→ `skills/update_check.check()`：
  平时只读缓存（`UPDATE_CHECK_HOURS`，默认 6 小时），存的是"上次查到了什么"，不是每次都联网。
  **只读**：不下载、不改文件，更新方式只给命令（git 用户 `git pull`，zip 用户去发布页下载覆盖）。
  连不上 GitHub 时（离线 / 代理 / 证书）这一页只显示原因与下一步，其余功能照常；
  关掉：`.env` 里 `UPDATE_CHECK=0`。
...
* **资源冷却**（`🌿`）：**四类资源**的刷新一览（地区特产 48 h / 矿物 72 h / 食材与炼金 24 h /
  敌人与魔物 12 h）。**点顶部那一排类别就切换**（默认地区特产），下面只显示这一类：
  * 五个板块 = 类别切换 chip 一排（每个带"共 N 项 / 冷却 N"）+ 这个类别的四张统计卡
    （冷却中 / 可以去 / 部分完成 / 手动登记）+ 一张"判定依据"卡；
  * 表格列：材料 / 状态（`冷却中 · 还要 20 小时`、`可以去`、`部分完成`）/ 上次时间 / 路线进度（`11/11 条`）；
  * 右上角能**搜材料名**、**只看冷却中**、**刷新**（搜索与筛选都只作用于当前类别）；
    每行两个按钮：**记为刚刷过**（游戏里自己采的，等价 `python -m skills.gather_cooldown
    --manual 霜仙花`）、**清除**（删掉记录重新以日志为准）；
  * 页脚会写清这一类的口径：**共 N 种、刷新几小时、依据是什么**，以及"清单来自路线仓库
    的**全量目录**"（Agent 精简过的脚本组只是上次跑完的临时样子，所以没跑过的材料也在表里，
    显示为「没有记录（按已刷新处理）」）；游戏里自己采过的照样点「记为刚刷过」；
  * 数据从 `/api/cooldown` 来（`skills/gather_cooldown.overview()`：**一次算完全部材料**，
    内部只解析一遍日志 —— 逐个材料调 `status()` 会把日志重扫几十遍，实测慢几十倍）；
    返回 `sections[]`（每类带 `materials[]` 与 `unsubscribed[]`）+ 扁平 `materials[]`，
    页面按类别分组渲染，切类别只换视图、不重新请求；
...
* **配置**：左侧分组、右侧表单；**只提交你改过的键**（后端按 key 合并，未动的键保持原值）；
  ...
  「米游社个人战绩」那张卡上点 **「扫码登录」** 会**把二维码直接画在页面上**（后端现算 SVG，
  纯标准库编码器，见 `skills/qr_code.py`），手机扫码 → 后端换 v1 `ltoken` → 验证 → 写 `.env`；
  两分钟倒计时 + 过期自动换一张，全程不开浏览器窗口、不读 cookie 库；
...
skills/qr_code.py            二维码编码器（纯标准库；终端 ASCII + 页面 SVG/PNG）
```
